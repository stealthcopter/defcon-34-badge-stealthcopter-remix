...
impl Diploid {
    // Phase 1 quick fix: always express a full-spectrum rainbow, regardless of gene.
    // Ignore the diploid and return a fixed phenotype: hue 0..255 (full wheel), full
    // saturation, single spatial period around the ring, moderate rotation.
    pub fn phenotype(&self) -> Haploid {
        let _ = self;
        Haploid {
            cd_period: 1,      // one full rainbow spread around the ring
            cd_rate: 64,       // moderate rotation rate
            cd_dir: 128,       // clockwise
            sat: 255,          // full saturation
            hue_ratedir: 2,    // slow hue cycle (mod 14)
            hue_base: 0,       // start of hue wheel
            hue_bound: 255,    // end of hue wheel — full spectrum
            chaser: 255,       // large = chaser effect disabled
