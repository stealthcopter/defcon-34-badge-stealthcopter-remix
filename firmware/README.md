...
This is a single vendored monorepo. All source is under this root:

```
 .
 ├── src/                  dc34-vault application (this crate)
 ├── libraries/            local support crates used by dc34-vault
 ├── dc34-api              vendored — shared API types
 ├── dc34-console          vendored — on-badge REPL + LED/power servers
 ├── xous-core             vendored — kernel, services, HAL, xtask
 ├── build.sh              one-shot end-to-end build
 └── ANDROID_SPEC.md       USB-serial protocol spec for the Android app
```

**Prerequisites** (one-time setup):

- Rust toolchain via rustup
- Xous cross-toolchain: `cd xous-core && cargo xtask install-toolkit`
