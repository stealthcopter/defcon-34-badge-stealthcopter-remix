                ...
                for &(conn, listener_op) in listeners.iter() {
                    if key != '\u{0000}' {
                        if key >= '\u{f700}' && key <= '\u{f8ff}' {
                            ...
                            .unwrap_or_else(|_| {
                                // Demoted from info! to debug! — this fires per-char during
                                // fast serial uploads (image/imagedc base64 chunks) because
                                // every char is broadcast to all listeners including the
                                // vault, whose message queue can't drain that fast. The vault
                                // doesn't need these keys during upload, so dropping them is
                                // correct behavior — just don't spam the info log about it.
                                log::debug!("Input overflow to {}, dropping keys!", conn);
                                xous::Result::Ok
                            });
                        }
                    }
