    ...
    */
    ...
    pub fn force(&mut self, phenotype: Haploid) {
        ...
        for (index, mrna) in mrnas.iter().enumerate() {
            // The 0x4000_0000 bit is required — the BIO coprocessor's dispatch
            // (main.c:471) only accepts genome writes when it is set. Without it,
            // codons are silently discarded and `force` has no effect. This matches
            // what `express()` sends, minus the pre/post zero-word paranoia.
